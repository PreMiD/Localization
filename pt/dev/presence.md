...
# Diretrizes
> Se você não seguir todas as diretrizes, um `Verificador de presença` solicitará as alterações adequadas ou sua solicitação de recebimento poderá até ser fechada em determinadas circunstâncias. 
> 
> {.is-warning}

> Ao fazer solicitações pull sobre como adicionar ou modificar presenças existentes, você <strong x-id = "1"> DEVE </strong> incluir uma captura de tela. No entanto, modificações nos arquivos ` metadata.json ` ou ` tsconfig.json ` de uma presença não requerem uma captura de tela. <em x-id = "3"> Sua captura de tela DEVE ser carregada diretamente no GitHub com a solicitação pull, não use sites de compartilhamento de imagens de terceiros. </em> 
> 
> {.is-warning}
...
- A presença <strong x-id = "1"> DEVE </strong> estar relacionada ao site que você escolheu.
- A presença <strong x-id = "1"> NÃO DEVE </strong> ser de nenhum site ilegal. Estes incluem sites pertubadores, drogas, pornografia infantil, etc...
